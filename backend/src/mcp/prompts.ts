...
function createSystemPrompt({
  ...
  const systemPrompt = `
    ...
    **Secondary role:** Review collection and FAQ responses.
    ...
    7. **Upsell Suggestion** (HARD): After options are collected, call the tool \`get_upsell_products\` to retrieve potential upsell items. If the tool returns any upsell products, present them to the customer and allow them to add items to the order. (e.g Would you like to add)
