...
userPermissionRoute.post('/update-permission', adminAuthMiddleware, async (req, res) => {
  try {
    const data = await permissionsService.updatePermission(req.body);
    const response: APIResponseFormat<any> = {
...
userPermissionRoute.get('/org-get-permissions', authMiddleware, async (req, res) => {
  try {
    const data = await permissionsService.getPermissions();
    const response: APIResponseFormat<any> = {
      message: 'permissions retreived successfully',
