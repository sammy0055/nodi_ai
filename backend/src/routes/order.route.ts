...
orderRoute.get('/order-stats-per-assigned-user', authMiddleware, async (req, res) => {
  try {
    const assignedUserId = req.query.assignedUserId as string;
    const data = await OrderService.getOrderStatsPerAsignedUser(assignedUserId, req.user!);
    const response: APIResponseFormat<any> = {
      message: 'order stats retrieved successfully',
      data,
    };
